...
// go test -v github.com/ulrichSchreiner/geo
...
func TestLineObservers(t *testing.T) {
	...
	if pOrigin.LineTo(pHoriz).IsVertical() {
		t.Errorf("horiz triggered vert")
	}
	if pOrigin.LineTo(pDiag).IsVertical() {
		t.Errorf("diag triggered vert")
	}
	if !pOrigin.LineTo(pVert).IsVertical() {
		t.Errorf("vert didn't trigger vert")
	}

	if !pOrigin.LineTo(pOrigin).IsDegenerate() {
		t.Errorf("not degenerate: %s", pOrigin.LineTo(pOrigin))
	}
	if pOrigin.LineTo(pDiag).IsDegenerate() {
		t.Errorf("degenerate")
	}
}
